...
import { Search, Plus, ArrowDown, Zap, Cpu } from 'lucide-react';
...
export function Hero() {
  return (
    ...
    >
      ...
      {/* ===== FOREGROUND — left control panel ===== */}
      <DepthLayer depth={100} className="absolute left-6 bottom-24 hidden xl:flex flex-col gap-3 pointer-events-auto">
        <div className="surface-metal-brushed rounded-xl p-3 flex flex-col items-center gap-2.5 relative metal-scratches">
          <Rivet size={6} className="absolute top-1 left-1" />
          <Rivet size={6} className="absolute top-1 right-1" />
          <Rivet size={6} className="absolute bottom-1 left-1" />
          <Rivet size={6} className="absolute bottom-1 right-1" />
          <span className="font-technical text-[7px] text-ink-3">UNIT 01</span>
          <MachineSwitch on label="PWR" size={22} />
          <MachineSwitch on={false} label="AUX" size={22} />
          <LEDMeter level={3} count={4} color="amber" size={4} />
        </div>
      </DepthLayer>
      ...
      <ForegroundLayer depth={60} className="relative z-20 min-h-screen flex items-center px-6 sm:px-12 lg:px-20 pt-28 pb-20">
        <div className="max-w-2xl">
          ...
          {/* machine controls */}
          <div className="mt-10 flex flex-col sm:flex-row items-start gap-4 anim-reveal" style={{ animationDelay: '0.45s' }}>
            <TactileButton variant="amber">
              <Search size={14} />
              Find a Jugaad
            </TactileButton>
            <TactileButton variant="mint">
              <Plus size={14} />
              Post a Jugaad
            </TactileButton>
          </div>
          ...
          {/* counter — subtle */}
          <div className="mt-10 flex items-center gap-3 anim-reveal" style={{ animationDelay: '0.6s' }}>
